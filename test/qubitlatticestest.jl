...
@testset "QubitLattice index basics" begin
    ...
    @test Set(get_qubits(ql)) == Set(qubits)
    @test get_qubits(ql, a1) == qubits
    ...
    @test get_indices(ql, 1) == [a1]
    @test get_indices(ql, 2) == [a1]
    @test get_indices(ql, 3) == [a1]
...
@testset "QubitLattice pairing basics" begin
    ...
    # check qubit -> index mapping
    @test Set(get_indices(ql, 3)) == Set([a1, b2])
    @test Set(get_indices(ql, 1)) == Set([a1])
    @test Set(get_indices(ql, 5)) == Set([b2])
...
@testset "QubitLattice multiple indices" begin
    ...
    @test Set(get_indices(ql, 2)) == Set([i1, i2])
    @test Set(get_indices(ql, 3)) == Set([i2, i3])
    @test Set(get_indices(ql, 1)) == Set([i3, i1])
    @test Set(get_indices(ql, 4)) == Set([i1])
    @test Set(get_indices(ql, 5)) == Set([i2])
    @test Set(get_indices(ql, 6)) == Set([i3])
...
@testset "QubitLattice extraction no unpaired" begin
    ...
    for (vals, ref) in zip(test_indices, ref_qubitvals)
        ...
        _, vals1 = qubitvals2idxvals(ql, qubitvals; inds=inds)
        @test vals == vals1
    end
end
...
@testset "QubitLattice 0 qubit" begin
    # triangle
    ql = QubitLattice()
    i1 = IndexLabel(1, :q)
    i2 = IndexLabel(2, :q)
    ...
    @test_throws ArgumentError idxval2qubitvals(ql, i1, 5)
    @test idxvals2qubitvals(ql, [i1, i2, i3], [3, 2, 4]) == Dict(0 => 0, 1 => 1, 2 => 1, 3 => 1)
    @test_throws ArgumentError idxvals2qubitvals(ql, [i1, i2, i3], [3, 4, 4])
end
