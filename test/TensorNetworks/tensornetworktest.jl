...
@testset "TensorNetwork construction" begin
    tn = TensorNetwork()
    @test length(get_tensors(tn)) == 0
    @test length(get_contractions(tn)) == 0
end
...
@testset "TensorNetwork basics" begin
    ...
    @test get_contraction(tn, c1) == ic2
end
...
@testset "TensorNetwork find" begin
    ...
    # find generic
    @test Set(find_indices(tn) do idx
        idx.group == 3 || idx.group == 5
    end) == Set([a3, b5])
...
@testset "TensorNetwork combine!" begin
    ...
    @test length(get_tensors(tn2)) == 2 * length(get_tensors(tn1))
    @test length(get_contractions(tn2)) == 2 * length(get_contractions(tn1))

    # matchcombine!
    # tn3 = deepcopy(tn1)
    # gmap = matchcombine!(tn3, tn2)
    # @test length(get_tensors(tn3)) == 3 * length(get_tensors(tn1))
    # @test length(get_contractions(tn3)) == 3 * length(get_tensors(tn1)) + 4
    # TODO
end
